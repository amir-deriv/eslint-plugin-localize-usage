...
function checkLocalizeComponentUsage(componentProps = [], context, node) {
  ...
  if (isInvalidTemplateLiteral(i18nDefaultPropNode.value.expression)) {
    reportInvalidTemplateLiteral(i18nDefaultPropNode.value.expression, context);
    return;
  }
  ...
  if (identifiersListInString.length > 0) {
    if (!valuesPropNode) {
      reportMissingProperties(node, identifiersListInString, context);
      return;
    }
...
function reportInvalidTemplateLiteral(expression, context) {
  context.report({
    node: expression,
    loc: expression.loc,
