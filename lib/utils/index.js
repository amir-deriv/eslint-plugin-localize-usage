...
function isValidUsageOfStringLiteral(componentProps = []) {
  ...
  console.log(i18nDefaultPropNode.value.expression)

  if((i18nDefaultPropNode.value.expression || {}).type === 'TemplateLiteral' && i18nDefaultPropNode.value.expression.expressions.length > 0) {
    return false;
  }
