"use strict";
function validateLocalizeArguments(localizeArguments = [], context) {
  localizeArguments.forEach((currentArg, index) => {
    if (index === 0) {
      if (currentArg.type === "BinaryExpression") {
        if (currentArg.left.type === "Identifier") {
          context.report({
            node: currentArg.left,
            loc: currentArg.left.loc,
            messageId: "onlyStringLiteralArgument",
          });
        }

        if (currentArg.right.type === "Identifier") {
          context.report({
            node: currentArg.right,
            loc: currentArg.right.loc,
            messageId: "onlyStringLiteralArgument",
          });
        }
